 ...
 * @version 1.0
 */
class  Saetclientv2 extends CI_Model {
	/**
	 * 构造函数
	 * 
	 * @access public
	 * @param mixed $akey 微博开放平	台应用APP KEY
	...
	public function init( $akey, $skey, $access_token, $refresh_token = NULL)
	{
		$this->load->model('Saetoauthv2');
		$o = new $this->Saetoauthv2();
		$o->init($akey, $skey, $access_token, $refresh_token);
	 ...
	 * 
	 <br />注意：为防止重复提交，当用户发布的微博消息与上次成功发布的微博消息内容一样时，将返回400错误，给出错误提示：
	 “40025:Error: repeated weibo text!“。 
	...
	function update( $status, $lat = NULL, $long = NULL, $annotations = NULL )
	{
		...
		if ($lat) 
		{
			$params['lat'] = floatval($lat); 	//获取纬度的浮点值
		}
		if ($long) 
		{
			$params['long'] = floatval($long);	//获取经度的浮点值
		}
